...
ATA::ATA(const bool $master, const u16 port_base)
    : data_port(port_base), error_port(port_base + 0x1), sector_count_port(port_base + 0x2), lba_low_port(port_base + 0x3), lba_mid_port(port_base + 0x4), lba_hi_port(port_base + 0x5), device_port(port_base + 0x6), command_port(port_base + 0x7), control_port(port_base + 0x206)
{
    master = $master;
}
...
void ATA::read_28(const u32 sector_number, const i32 count)
{
...
void ATA::write_28(const u32 sector_number, const u8 *data, const u32 count)
{
    if (sector_number > 0x0FFFFFFF)
    {
        return;
