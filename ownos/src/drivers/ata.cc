...
AdvancedTechnologyAttachment::AdvancedTechnologyAttachment(bool master, shared::uint16_t portBase)
    : dataPort(portBase),
...
void AdvancedTechnologyAttachment::Read28(shared::uint32_t sectorNum, int count)
{
...
void AdvancedTechnologyAttachment::Write28(shared::uint32_t sectorNum, shared::uint8_t *data, shared::uint32_t count)
{
    if (sectorNum > 0x0FFFFFFF)
        return;
    if (count > 512)
