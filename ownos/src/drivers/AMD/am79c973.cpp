...
amd_am79c973::recv()
{
	...
	for (;
		 (recv_buffer_descriptor[current_recv_buffer].flags & 0x80000000) == 0;
		 current_recv_buffer = (current_recv_buffer + 1) % 8)
	{
		...
		{
			u32 size =
			  recv_buffer_descriptor[current_recv_buffer].flags & 0xFFF;
			if (size > 64)
			{
				size -= 4;
			}
			...
			u8* buffer =
			  (u8*)(recv_buffer_descriptor[current_recv_buffer].address);
