...
bool RawDataHandler::on_rawdata_recv(u8 *buffer, u32 size)
{
  return false;
}

void RawDataHandler::send(u8 *buffer, u32 size)
{
...
amd_am79c973::amd_am79c973(const PCIDeviceDescriptor *device,
    ...
    : Driver(), InterruptHandler(interrupt_manager,
                                 device->interrupt +
                                     interrupt_manager->hardware_interrupt_offset),
      mac_address0_port(device->port_base), mac_address2_port(device->port_base + 0x02), mac_address4_port(device->port_base + 0x04), register_data_port(device->port_base + 0x10), register_address_port(device->port_base + 0x12), reset_port(device->port_base + 0x14), bus_control_register_data_port(device->port_base + 0x16)
{
  handler = 0;
...
void amd_am79c973::send(u8 *buffer, usize size)
{
...
void amd_am79c973::set_handler(RawDataHandler *$handler)
{
  handler = $handler;
}
...
void amd_am79c973::set_ip_address(const u32 ip)
{
