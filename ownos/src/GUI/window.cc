...
void Window::on_mousedown(i32 x, i32 y, u8 button)
{
  dragging = button == 1;
  CompositeWidget::on_mousedown(x, y, button);
}

void Window::on_mouseup(i32 x, i32 y, u8 button)
{
  dragging = false;
  CompositeWidget::on_mouseup(x, y, button);
}

void Window::on_mousemove(i32 old_x, i32 old_y, i32 new_x, i32 new_y)
{
  if (dragging)
  {
    x += new_x - old_x;
