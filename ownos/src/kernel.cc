...
printf(char* str)
{
	// L'utilisation du mot clé static sur une variable locale change sa durée
	// de durée automatique à durée statique. Cela signifie que la variable est
	...
	for (usize i = 0; str[i] != '\0'; ++i)
	{
		switch (str[i])
		{
			...
			default:
				video_memory[80 * y + x] =
				  (video_memory[80 * y + x] & 0xFF00) | str[i];
				x++;
				break;
		}
...
kernel_main(void* multiboot_struct, u32 magicnumber)
{
	...
	*/
	...
	u32 ip_be =
	  ((u32)ip4 << 24) | ((u32)ip3 << 16) | ((u32)ip2 << 8) | (u32)ip1;
