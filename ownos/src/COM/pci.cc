...
u32 PCIController::read(const u16 bus,
                        const u16 device,
                        const u16 fn,
...
void PCIController::write(const u16 bus,
                          const u16 device,
                          const u16 fn,
                          const u32 register_offset,
                          const u32 value)
...
bool PCIController::device_has_functions(const u16 bus, const u16 device)
{
    return read(bus, device, 0, 0x0E) & (1 << 7);
}

void PCIController::select_drivers(const DriverManager *driver_manager,
